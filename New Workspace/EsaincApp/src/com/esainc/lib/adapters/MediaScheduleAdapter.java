...
import com.esainc.lib.ui.MediaScheduleFragment;
import com.esainc.lib.ui.TeamsActivity;
import com.nostra13.universalimageloader.core.ImageLoader;

public class MediaScheduleAdapter extends BaseAdapter{
	private static final String LOG_TAG = "MediaSchedule Adapter";
	private Context mContext;
	private JSONArray mData;
	private boolean mTeams;
	private ImageLoader imageLoader = ImageLoader.getInstance();
	public MediaScheduleAdapter(Context context,JSONArray mSchedule, boolean teams) {
		mContext = context;
		mData = mSchedule;
		mTeams = teams;
	}
	...
	public long getItemId(int pos) {
		try {
			return Long.valueOf(mData.getJSONObject(pos).getString(mContext.getResources().getString(R.string.tag_scheduleID))).longValue();
		...
		try {
			...
			// Note
             wvNote.loadUrl("about:blank");
				if (!TextUtils.isEmpty(hdata.getString(mContext.getResources().getString(R.string.note)).trim())) {
					...
					MediaScheduleFragment.getInstance();
					wvNote.setWebViewClient(new WebViewListener(MediaScheduleFragment.getInstance(), MediaScheduleFragment.mPos, mTeams, null));
					
					String data = "<html><head><style type=\"text/css\"> body {font-family: \"helvetica\"; font-size: 14;} a { color:#" + 
							mContext.getResources().getString(R.color.HyperlinksColor).substring(3) +
							"; text-decoration:none; } </style><meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=1.0'></head><body>" + 
							hdata.getString(mContext.getResources().getString(R.string.tag_note)) + "<body></html>";
			...
			if (hdata.getString(mContext.getResources().getString(R.string.tag_played)).equalsIgnoreCase("Not Played")) {
				...
				imgCalendar.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						try {
							...
							MediaScheduleFragment.getInstance().startActivity(intent);
						} catch (ParseException e) {
				...
				imgVideo.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						Intent browserIntent = new Intent(MediaScheduleFragment.getInstance().getActivity(), BrowserActivity.class);
						browserIntent.putExtra("pos", MediaScheduleFragment.mPos);
						...
						MediaScheduleFragment.getInstance().startActivity(browserIntent);
					}
				});
				...
				imgRecap.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						Intent dataIntent = new Intent(MediaScheduleFragment.getInstance().getActivity(), DataLoader.class);
						Messenger dataMessenger = new Messenger(MediaScheduleFragment.mHandler);
						dataIntent.putExtra("MESSENGER", dataMessenger);
						dataIntent.putExtra("tabPos", MediaScheduleFragment.mPos);
						...
						MediaScheduleFragment.getInstance().getActivity().startService(dataIntent);
					}
				});
