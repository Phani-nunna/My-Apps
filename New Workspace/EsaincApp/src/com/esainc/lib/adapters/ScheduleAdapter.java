...
import com.esainc.lib.ui.ScheduleFragment;
import com.esainc.lib.ui.TeamsActivity;
import com.nostra13.universalimageloader.core.ImageLoader;

public class ScheduleAdapter extends BaseAdapter {
	private static final String LOG_TAG = "Schedule Adapter";
	private Context mContext;
	private JSONArray mData;
	private boolean mTeams;
	private ImageLoader imageLoader = ImageLoader.getInstance();
	
	public ScheduleAdapter(Context context, JSONArray data, boolean teams) {
		mContext = context;
		mData = data;
		mTeams = teams;
	}
		...
		try {
			...
			// Note
             wvNote.loadUrl("about:blank");
				if (!TextUtils.isEmpty(hdata.getString(mContext.getResources().getString(R.string.note)).trim())) {
					...
					wvNote.setWebViewClient(new WebViewListener(ScheduleFragment.getInstance(), ScheduleFragment.mPos, mTeams, null));
			...
			if (hdata.getString(mContext.getResources().getString(R.string.tag_played)).equalsIgnoreCase("Not Played")) {
				...
				imgCalendar.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						try {
							...
							ScheduleFragment.getInstance().startActivity(intent);
						} catch (ParseException e) {
				...
				imgVideo.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						Intent browserIntent = new Intent(ScheduleFragment.getInstance().getActivity(), BrowserActivity.class);
						browserIntent.putExtra("pos", ScheduleFragment.mPos);
						...
						ScheduleFragment.getInstance().startActivity(browserIntent);
					}
				});
				...
				imgRecap.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						Intent dataIntent = new Intent(ScheduleFragment.getInstance().getActivity(), DataLoader.class);
						Messenger dataMessenger = new Messenger(ScheduleFragment.mHandler);
						dataIntent.putExtra("MESSENGER", dataMessenger);
						dataIntent.putExtra("tabPos", ScheduleFragment.mPos);
						...
						ScheduleFragment.getInstance().getActivity().startService(dataIntent);
					}
				});
