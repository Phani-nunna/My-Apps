...
public class UnFollowUserProfileAsyncTask extends
	...
	public UnFollowUserProfileAsyncTask(Context context, Button view,
	...
	protected FollowResponseDto doInBackground(Void... params) {
		...
		followResponse = Utils
				.makePostRequest(Constants.USER_UNFOLLW, followJson);
	...
	protected void onPostExecute(FollowResponseDto result) {
		...
		if (result != null) {
			...
			try {
				...
				{
					followingUsers.remove(foodiesObjectId);
				}
			...
			new SvaadDialogs().showToast(context,"Unfollowed successfully");
