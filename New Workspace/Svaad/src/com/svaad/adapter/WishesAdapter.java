...
public class WishesAdapter extends ArrayAdapter<FeedDetailDto> {
	...
	public WishesAdapter(Context context, int gridviewId, GridView grid,
			List<FeedDetailDto> feedList) {
	...
	public View getView(int arg0, View view, ViewGroup group) {
		...
		if (view == null) {
			...
			view = inflater.inflate(R.layout.wishthumbnail, group, false);
		...
		DishPhotoDto dishPhoto = feed.getDishPhotoThumbnail();
...
//		if (branchDishId != null) {
			...
			long ateitCount = feed.getComments();
...
//		}
		...
		holder.btnAteIt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (Utils.getFromSharedPreference(Constants.USER_OBJECT_ID_KEY) != null
					...
					ateItIntent.putExtra(Constants.SVAAD, Constants.WISHLIST);
					context.startActivity(ateItIntent);
				} else {
					new SvaadDialogs().showSvaadLoginAlertDialog(context, null,
							Constants.RES_MODE,Constants.WISHLIST,Constants.ATE_IT);
				}
			}
		});
		// holder.userRl.setOnClickListener(new OnClickListener() {
		//
		// @Override
		// public void onClick(View v) {
		// FeedDetailDto feedDetailDto = (FeedDetailDto) v.getTag();
		...
		holder.btnAddToList.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (Utils.getFromSharedPreference(Constants.USER_OBJECT_ID_KEY) != null
										...
										if (wishlists != null
											...
											try {
												...
												new LikeAsyncTask(context,
														null, feedDetailDto,Constants.WISHLIST)
														.execute();
											} catch (Exception e) {
												e.printStackTrace();
											}
										...
										for (int i = 0; i < wishlists.size(); i++) {
											...
											if (id != null && id.length() > 0) {
												if (id.equals(branchDishObjectId)) {
													...
													new UnlikeAsyncTask(
															context, null,
															branchDishObjectId,Constants.WISHLIST,feedDetailDto)
															.execute();
					...
					new SvaadDialogs().showSvaadLoginAlertDialog(context, null,
							Constants.RES_MODE,Constants.WISHLIST,Constants.WISH_IT);
				}
	...
	public class ViewHolder {
		...
		private TextView tvCommenttext, tvDishName, tvResName, tvLocationName,
				tvDishTag,tvAteitCount;
